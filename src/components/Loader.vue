  ...
  <div class="app-container">
    ...
    <Transition name="fade">
      <div class="loading-overlay" v-if="isLoading">
        <div class="logo-container" :class="{ loaded: loadingProgress >= 100 }">
          <svg
            width="50"
            height="50"
            viewBox="0 0 33 33"
    ...
    <div class="demo-controls" v-if="isDemoMode">
      <button @click="startLoading" class="control-button">
        Start Loading
      </button>
    </div>
  </div>
...
const startLoading = () => {
  ...
  const endTime = startTime + props.loadingDuration;

  const updateProgress = () => {
    const now = Date.now();
    const elapsed = now - startTime;
    const progress = Math.min(100, (elapsed / props.loadingDuration) * 100);
...
.control-button {
  ...
  font-size: 14px;
  font-weight: 500;
  box-shadow: 0 2px 8px rgba(70, 64, 222, 0.3);
...
.control-button:hover {
  ...
  box-shadow: 0 4px 12px rgba(70, 64, 222, 0.4);
}
