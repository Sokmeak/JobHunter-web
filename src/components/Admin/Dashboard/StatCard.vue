<template>
  <div class="stat-card">
    ...
    <div class="card-footer" :class="changeTypeClass">
      <span class="card-status" v-if="status === 'increase'">↑ % </span>
      <span class="card-status" v-else-if="status === 'decrease'">↓ %</span>
      <span class="card-status" v-else-if="status === 'neutral'">→ %</span>
      <span>{{ change }}</span>
      <span v-if="changeText">{{ changeText }}</span>
    </div>
  </div>
</template>
...
const changeTypeClass = computed(() => {
  return {
    ...
    warning: props.changeType === "warning",
