        ...
        <div v-if="selectedConversation" class="chat-container">
          ...
          <div class="chat-header">
            ...
            <div class="chat-actions">
              <button class="action-btn" @click="markAsRead" title="Mark as read">
                <span>✓✓</span>
              </button>
              <button class="action-btn" @click="archiveConversation" title="Archive">
                <span>📁</span>
              </button>
              <button class="action-btn" @click="showMoreActions = !showMoreActions" title="More">
                <span>⋮</span>
              </button>
              <div v-if="showMoreActions" class="dropdown-menu">
                <button @click="blockContact">Block Contact</button>
                <button @click="deleteConversation" class="text-danger">Delete Conversation</button>
              </div>
            </div>
          </div>
          ...
          <div class="messages-container" ref="messagesContainer">
            ...
            <div v-if="selectedConversation.messages.length === 0" class="conversation-starter">
              <div class="starter-content">
                <div class="starter-avatar">
                  <img :src="selectedConversation.avatar" :alt="selectedConversation.name" />
                </div>
                <h3 class="starter-name">{{ selectedConversation.name }}</h3>
                <p class="starter-text">Start your conversation with {{ selectedConversation.name.split(' ')[0] }}</p>
          ...
          <div class="chat-input-bar">
            ...
            <div v-if="newMessage.trim()" class="message-actions">
              <div class="message-preview">
                <span class="character-count">{{ newMessage.length }}/1000</span>
              </div>
              <div class="quick-actions">
                <button class="quick-action-btn" @click="clearMessage" title="Clear">
                  <span>✕</span>
                </button>
                <button class="quick-action-btn" @click="formatBold" title="Bold">
                  <span><strong>B</strong></span>
                </button>
                <button class="quick-action-btn" @click="formatItalic" title="Italic">
                  <span><em>I</em></span>
                </button>
              </div>
            </div>
    ...
    <div v-if="showNewMessageModal" class="modal-overlay" @click="closeNewMessageModal">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" @click="closeNewMessageModal">Cancel</button>
          <button 
            type="button" 
            class="btn btn-primary"
...
export default {
  ...
  data() {
    return {
      ...
      contacts: [
        { id: 101, name: 'Sarah Johnson', title: 'HR Manager at TechCorp', avatar: 'https://images.unsplash.com/photo-1494790108755-2616b612b786?w=150&h=150&fit=crop&crop=face' },
        { id: 102, name: 'Michael Chen', title: 'Recruiter at InnovateTech', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&h=150&fit=crop&crop=face' },
        { id: 103, name: 'Emily Rodriguez', title: 'Talent Acquisition at GlobalSoft', avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=150&h=150&fit=crop&crop=face' },
        { id: 104, name: 'David Kim', title: 'Technical Recruiter at DevHub', avatar: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop&crop=face' }
      ]
    };
  },
  computed: {
    filteredConversations() {
      ...
      this.filters[0].count = this.conversations.filter(c => !c.archived).length;
      this.filters[1].count = this.conversations.filter(c => c.unreadCount > 0).length;
      this.filters[2].count = this.conversations.filter(c => c.archived).length;
    ...
    canSendMessage() {
      return this.newMessage.trim().length > 0 && this.newMessage.length <= 1000;
    }
  },
  methods: {
    formatTime(date) {
    ...
    getInputPlaceholder() {
      ...
      if (this.selectedConversation.messages.length === 0) {
        return `Say hello to ${this.selectedConversation.name.split(' ')[0]}...`;
      }
    ...
    simulateResponse() {
      ...
      setTimeout(() => {
        ...
        if (isFirstResponse) {
          ...
          responseText = welcomeResponses[Math.floor(Math.random() * welcomeResponses.length)];
        } else {
    ...
    createNewConversation() {
      ...
      const existingConv = this.conversations.find(c => c.name === this.selectedContact.name);
