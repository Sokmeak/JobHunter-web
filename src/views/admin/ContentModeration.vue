<template>
    <div class="content-moderation">
      <div class="page-header">
        <div class="header-content">
          ...
          <p class="page-subtitle">Review job postings and handle reported content</p>
        </div>
      </div>
      ...
      <div class="pagination">
        <div class="pagination-info">
          Showing {{ startIndex + 1 }} to {{ endIndex }} of {{ totalContent }} items
        </div>
  ...
  const filteredContent = computed(() => {
    return contentItems.value.filter(item => {
      ...
      const matchesStatus = !statusFilter.value || item.status.toLowerCase() === statusFilter.value.toLowerCase();
      const matchesType = !typeFilter.value || item.type.toLowerCase() === typeFilter.value.toLowerCase();
      const matchesReports = !reportsFilter.value || checkReportsFilter(item.reports);
  ...
  const endIndex = computed(() => Math.min(startIndex.value + itemsPerPage, totalContent.value));
  
  // Methods
  const checkReportsFilter = (reports) => {
    switch (reportsFilter.value) {
      case 'none': return reports === 0;
      case 'low': return reports >= 1 && reports <= 2;
      case 'medium': return reports >= 3 && reports <= 5;
      case 'high': return reports > 5;
      default: return true;
    }
  };
  ...
  const confirmAction = (actionData) => {
    const content = contentItems.value.find(item => item.id === selectedContent.value.id);
    if (content) {
